...
A dashboard is usually treated as a destination — a place you go to check on things. But think about what "having to check" actually means. The system can't tell you when something matters, so it makes you watch, constantly, just in case. A dashboard you have to monitor has quietly offloaded its hardest job onto your attention. The best version of one barely exists at all. It stays dark until there's a reason not to.

## Watching is a failure mode, not a feature

The instinct to build a rich, always-on dashboard comes from a good place — visibility. But visibility you have to babysit isn't visibility. It's surveillance duty with a nicer name. The better design inverts it: silence by default, signal by exception. Don't make me watch a number. Tell me when the number needs me.
...
This is really the same move as turning a spreadsheet into a dashboard in the first place — collapsing a wall of raw numbers into the one thing you actually need to see. A disappearing dashboard just takes that one step further and collapses it all the way down to nothing, until it matters. (I built and animated that first move literally in [Death by a Thousand Cells](/blog/death-by-a-thousand-cells/), and wrote about the "best design is invisible" idea more broadly in [The Design System Nobody Sees](/blog/the-design-system-nobody-sees/).)

None of this means data views are bad, to be clear. It means the default state should be quiet. Keep the rich view for when someone deliberately goes looking — the deep-dive, the real investigation. But the everyday state shouldn't require anyone to sit and watch, because a person watching a dashboard is a person the system already failed to notify.
...
<li><span class="tick" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M5 12.5l4 4L19 6.5"/></svg></span><span>Measure success by how rarely people need to open it. A tool that earns its own irrelevance is doing its job well.</span></li>
</ul>
