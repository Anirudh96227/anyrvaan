---
title: "Ten Apps, Rebuilt by Hand"
summary: "I rebuilt ten apps you actually use — Excel, Gmail, Slack, Stripe and more — from scratch, then set a fake mouse loose inside each one. It looks exactly like a screen recording, except nothing real is running."
year: "2026"
order: 9
...
The rule I set myself sounds like a joke: no shortcuts, no reusing pieces between apps — every row, tab, and button placed by hand, one at a time. That tedium is the entire secret. Build an app the fast, efficient way and everything comes out evenly spaced and a little generic, and a viewer clocks it as fake without quite knowing why. Real apps are messy, full of odd little choices real teams made over years, and the only way to copy that mess is to copy each choice by hand.

What sells it is the stuff nobody looks at directly. Drag across some cells in the Excel one and the little sum quietly updates at the bottom, exactly where it should be. In Slack, a reaction on a message ticks from one to two, as if a coworker just added theirs. In Gmail, replying collapses the thread and the "message sent" note slides in dark at the corner. None of those is the point of the shot — they're the details a viewer checks without knowing it, to decide whether what they're seeing is real. Miss them and the whole thing feels slightly off.

The mouse is the trick that ties it together. It doesn't teleport — it eases toward a button and slows down just before it lands, the way a real hand does, then gives a tiny press when it clicks. Why the tedious path was the right one — and why one click should ripple out and change several things at once — is [I did it the hard way on purpose](/blog/least-efficient-on-purpose/).
